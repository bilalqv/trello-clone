...
export default function AddNew({ type, parentId }) {
  ...
  return (
    <div className="">
      <button onClick={openForm}>Add New {type == 'card' ? 'Card' : 'Stage'} +</button>
      {showForm &&
        <form action="" onSubmit={submitHandler} className="mt-2" >
          <input type="text" value={inputTitle} onChange={updateInputTitle}
            placeholder={type == 'card' ? 'Enter Card Title' : 'Enter Stage Title'}
          ...
          {type == 'card' &&
            <>
              ...
              <p><span>Add Background Image: </span>
                <input type="checkbox" checked={addBackgroundImage} onChange={handleCheckboxChange}
